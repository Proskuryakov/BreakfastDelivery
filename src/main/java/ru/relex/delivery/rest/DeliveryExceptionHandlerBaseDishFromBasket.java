...
@ControllerAdvice
public class DeliveryExceptionHandlerBaseDishFromBasket {
    ...
    ResponseEntity<List<ErrorModel>> handleConstraintViolationException(ConstraintViolationException e) {
        ...
        final var errorModels = errors.stream()
                .map(err -> new ErrorModel(err.getMessage(), ValidationErrorsDishesFromBasket.getMessageByCode(err.getMessage())))
