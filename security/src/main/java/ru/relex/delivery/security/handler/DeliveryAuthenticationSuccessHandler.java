package ru.relex.delivery.security.handler;
...
public class DeliveryAuthenticationSuccessHandler implements AuthenticationSuccessHandler {

    private static final Logger logger = LoggerFactory.getLogger(DeliveryAuthenticationSuccessHandler.class);

    private final UserAuthMapper userMapper;
    protected static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    public DeliveryAuthenticationSuccessHandler(UserAuthMapper userMapper) {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if (!(object instanceof DeliveryUserDetails)) {
            logger.error("Expected UserDetails but got {}", object.getClass());
            throw new ServletException("Unexpected authorization class");
        }

        UserProfile profile = userMapper.getProfileInfo(((DeliveryUserDetails) object).getUsername());
